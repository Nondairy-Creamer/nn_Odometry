from tensorflow.keras.layers import multiply, Dropout, concatenate, LSTM, Dense, Input, Activation, BatchNormalization, Conv2D, subtract, add, Lambda
...
def ln_model(input_shape, filter_shape, num_filter=(8, 6), sum_over_space=True):
    ...
    # T4/T5
    # intial convolution
    conv1 = Conv2D(num_filter[0], filter_shape, strides=(1, 1), name='T4_T5',
    ...
    combine_filters = Conv2D(num_filter[1], (1, conv_x_size), strides=(1, 1), name='LPTC',
                             kernel_initializer=glorot_uniform(seed=None),
    ...
    combine_filters_d = Dropout(d_rate)(combine_filters)

    # behavior
    #behavior = Conv2D(1, (1, 1), strides=(1, 1), name='behavior',
    #                  kernel_initializer=glorot_uniform(seed=None))(combine_filters_d)

    # Create model
    model = Model(inputs=image_in, outputs=combine_filters_d, name='ln_model')

    return model, pad_x, pad_t, learning_rate, batch_size


def ln_model_deep(input_shape, filter_shape, num_filter=(8, 6), sum_over_space=True):
    learning_rate = 0.001*1
    batch_size = np.power(2, 6)
...
def hrc_model(input_shape, filter_shape, num_filter=(8, 6), sum_over_space=True):
    learning_rate = 0.001 * 1
    batch_size = np.power(2, 6)
    ...
    # T4/T5
    # intial convolution
    conv1_l = Conv2D(num_filter[0], filter_shape, strides=(1, 1), name='T4_T5_l',
                     kernel_initializer=glorot_uniform(seed=None),
    ...
    conv1_r = Conv2D(num_filter[0], filter_shape, strides=(1, 1), name='T4_T5_r',
                     kernel_initializer=glorot_uniform(seed=None),
                     kernel_regularizer=regularizers.l1(reg_val),
                     use_bias=False)(image_in)
    ...
    # Create model
    model = Model(inputs=image_in, outputs=combine_filters_d, name='hrc_model')
